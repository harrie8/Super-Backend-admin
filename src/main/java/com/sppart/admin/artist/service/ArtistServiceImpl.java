...
import com.sppart.admin.artist.domain.mapper.ArtistMapper;
import com.sppart.admin.artist.dto.*;
...
public class ArtistServiceImpl implements ArtistService{
    ...
    @Override
    @Transactional
    public ResponseArtists getAllArtists(Date startDate, Date endDate, boolean isUser, String searchString, FilterType filterType, int page) {
...
//        }
    ...
    public List<?> handleIdFilter(String searchString, Date startDate, Date endDate, int[] pageIndex, boolean isUser) {
        if (Objects.nonNull(startDate)) {
            return entityToResponse(artistMapper.getArtistsByDateAndInstagramId(searchString, startDate, endDate, pageIndex[0], pageIndex[1]));
        } else {
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW, isolation = Isolation.READ_COMMITTED)
    public List<?> handleNameFilter(String searchString, Date startDate, Date endDate, int[] pageIndex, boolean isUser) {
        if (Objects.nonNull(startDate)) {
            return entityToResponse(artistMapper.getArtistsByDateAndName(searchString, startDate, endDate, pageIndex[0], pageIndex[1]));
        } else {
